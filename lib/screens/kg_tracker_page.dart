...
class KgTrackerPage extends StatelessWidget {
  const KgTrackerPage({super.key});
  ...
  Color getTrafficColor(List<Map<String, dynamic>> entries, String goalMode) {
    ...
    final previous = getDouble(entries[entries.length - 2]["weight"]);
    final current = getDouble(entries.last["weight"]);
  ...
  String getTrafficText(List<Map<String, dynamic>> entries, String goalMode) {
    if (entries.length < 2) {
      return "Status: waiting for more data. Add at least two weight entries to compare progress.";
    }
    ...
    if (current == previous) {
      return "Status: stable. Your weight is the same as your previous entry.";
    }

    if (goalMode == "Slabit") {
      return current < previous
          ? "Status: on track. Your weight is going down, which matches your Weight Loss goal."
          : "Status: attention needed. Your weight increased compared to your previous entry.";
    }

    return current > previous
        ? "Status: on track. Your weight is going up, which matches your Muscle Gain goal."
        : "Status: attention needed. Your weight decreased compared to your previous entry.";
  }

  double getMinY(List<Map<String, dynamic>> entries) {
    final weights = entries.map((entry) => getDouble(entry["weight"])).toList();
    final minValue = weights.reduce((a, b) => a < b ? a : b);
    return minValue - 2;
  }

  double getMaxY(List<Map<String, dynamic>> entries) {
    final weights = entries.map((entry) => getDouble(entry["weight"])).toList();
    final maxValue = weights.reduce((a, b) => a > b ? a : b);
    return maxValue + 2;
  }

  String getLatestWeightText(List<Map<String, dynamic>> entries) {
    if (entries.isEmpty) return "--";
    final weight = getDouble(entries.last["weight"]);
                ...
                child: Container(
                  ...
                  child: const Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 10),
                      Text(
                        "Add your daily data from the Dashboard to start tracking your weight progress.",
                        textAlign: TextAlign.center,
                        style: TextStyle(
                      ...
                      BarChartData(
                        ...
                        barGroups: entries.asMap().entries.map((entry) {
                          return BarChartGroupData(
                            ...
                            barRods: [
                              BarChartRodData(
                                toY: getDouble(entry.value["weight"]),
                                color: green,
                                width: 18,
